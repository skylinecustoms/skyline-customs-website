...
The city × service landing pages (`/ppf-reston-va`, `/window-tinting-ashburn-va`, …)
are generated from one data file and one template:

- `client/src/lib/localSeo.ts` holds every city (drive time, local roads,
  neighborhoods, neighbor links) and every service's copy, features, and FAQs.
- `client/src/components/LocalServicePage.tsx` renders a page from that data.
- `client/src/pages/<City><Service>.tsx` are three-line wrappers that exist so
  each route gets its own lazy-loaded chunk.

**To add a city:** add an entry to `CITIES` and `CITY_ORDER` in `localSeo.ts`,
create the four wrapper files (copy any existing city's), register the four
routes in `client/src/App.tsx`, and add four entries to `STATIC_META` in
`server/_core/ssrMeta.ts` so the server sends the right title and description.
The sitemap, service-areas hub, footer, and neighbor links pick it up
automatically.

## SEO behavior worth knowing

- `sitemap.xml` is generated by the server from the route list and published
  blog posts in the database; there is no static file to edit.
- Unknown URLs return a real 404 status. Trailing-slash URLs 301 to the
  canonical form. Non-www and http redirect to `https://www.`.
- Titles, descriptions, and canonicals are injected server-side for every
  route so crawlers get them without running JavaScript.
- Build assets under `/assets` are cached for a year (they are content-hashed);
